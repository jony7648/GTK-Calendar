...
void create_labels(PropertyMap<int>& property_map, gtkc::Container* container) {
	...
	for (auto& [key, value] : property_map) {
		...
		key_map->set_halign(GTK_ALIGN_START);
...
static void signal_note_added(core::EmitData<data::NoteContainer> emit_data) {
	...
	auto* sig_data = static_cast<data::NoteContainer::SigNoteAdded*>(emit_data.sig_data);
	gtkc::GridContainer& main_container = scene->get_widget_container();


	gtkc::GridContainer& grid = scene->get_widget_container();

	str_property_map["LastNoteText"] = sig_data->back_note.text;
	str_property_map["StartIndex"] = 7;
	str_property_map["EndIndex"] = sig_data->end_index;
	refresh_property_label("LastNoteText", str_property_map, grid, sig_data->back_note.text);
	...
	int loop_index = sig_data->start_index;
...
core::Scene* create_debug_scene(ActivateSignal* activate_signal) {
	...
	note_container.sig_handler.add_emit_func(data::NoteContainer::S_NOTE_ADDED, &signal_note_added, debug_scene);
